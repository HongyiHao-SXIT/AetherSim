...
void Custom::RobotControl() {
  ...
  for (int idx = 0; idx < 12; idx++) {
    printf("motorstate.q = %lf\n", state.motorState[idx].q);
  }
  for (int foot_idx = 0; foot_idx < 4; foot_idx++) {
    printf("foot force = %d\n", state.footForce[foot_idx]);
  }
...
int main(void) {
  std::cout
      ...
      << "NOTE: The robot also needs to be set to LOW-level mode, otherwise it "
         "will make strange noises and this example will not run successfully! "
      << std::endl
