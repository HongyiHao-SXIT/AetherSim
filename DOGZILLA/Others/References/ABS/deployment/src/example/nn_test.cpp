...
void Custom::RobotControl() {
  ...
  if (motiontime >= 500) {
    float torque = (0 - state.motorState[FR_1].q) * 10.0f +
                   (0 - state.motorState[FR_1].dq) * 1.0f;
    if (torque > 5.0f)
      torque = 5.0f;
    if (torque < -5.0f)
      torque = -5.0f;
...
//         if (motiontime < 500){
//             for (i = 0; i < 12; i++ ){
//                 action[i] = base_pos[i] + (desired_pos[i] - base_pos[i]) *
//                 (motiontime - 100) / 400;
//             }
//         }
...
int main(void) {
  ...
  // LoopFunc loop_control("control_loop", custom.dt,
  // boost::bind(&Custom::RobotControl, &custom)); LoopFunc
  // loop_udpSend("udp_send",     custom.dt, 3, boost::bind(&Custom::UDPSend,
  // &custom)); LoopFunc loop_udpRecv("udp_recv",     custom.dt, 3,
  // boost::bind(&Custom::UDPRecv,      &custom)); std::cout << "before start"
  // << std::endl; loop_udpSend.start(); loop_udpRecv.start();
  // loop_control.start();
