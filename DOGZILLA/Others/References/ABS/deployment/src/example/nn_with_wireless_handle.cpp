...
std::array<float, 3> quat_rotate_inverse(std::array<float, 4> quat,
...
class Custom {
public:
  Custom(uint8_t level, double vel_x, double vel_y, double vel_w)
      : safe(LeggedType::Go1), udp(level, 8090, "192.168.123.10", 8007) {
    udp.InitCmdData(cmd);
    base_pos[FL_0] = 0.4;
    base_pos[FL_1] = 1.16;
    base_pos[FL_2] = -2.7;
    ...
    policy = torch::jit::load(
        "/home/tairanhe/workspace/sim2real-locomotion-deployment/"
        "unitree_legged_sdk/model/go1_apr10.pt");

    // policy =
    // torch::jit::load("/home/tairanhe/workspace/sim2real-locomotion-deployment/unitree_legged_sdk/model/policy_1.pt");
    ...
    // neural_vel_estimator =
    // torch::jit::load("../neural_vel_predictor_scripted.pt");
...
std::array<float, 12> Custom::clipSafeTorque(std::array<float, 12> desired) {
  ...
  for (i = 0; i < 12; i++) {
    clip_min = (torque_min + 0.5 * state.motorState[i].dq) / 40 +
               state.motorState[i].q;
    clip_max = (torque_max + 0.5 * state.motorState[i].dq) / 40 +
               state.motorState[i].q;
    if (desired[i] < clip_min)
      desired[i] = clip_min;
    if (desired[i] > clip_max)
      desired[i] = clip_max;
  }
  return desired;
}
...
void Custom::RobotControl() {
  ...
  // if (motiontime < 500+20) base_vel[0] = (motiontime - 500) * dt * 2 * 100 /
  // 20; else base_vel[0] = 2; base_vel[0] = (motiontime - 500) * dt;
  // base_vel[1] = 0;
  // base_vel[2] = 0;
  ...
  if (motiontime <= 100) {
    ...
    if (motiontime < 500) {
      for (i = 0; i < 12; i++) {
        action[i] = base_pos[i] +
                    (desired_pos[i] - base_pos[i]) * (motiontime - 100) / 400;
      }
      ...
      if (motiontime == 900) {
        ...
        // for (int i=0; i<4;i++) com_vel_estimator.foot_threshold[i] = 20;
        // //foot_threshold[i];
      }
      // for (i = 0; i < 12; i++ ){
      //     action[i] = desired_pos[i];
      //     action[i] += cos((motiontime - 400) * 2 * M_PI / 50) *
      //     dof_class_sin[i]; action[i] -= sin((motiontime - 400) * 2 * M_PI /
      //     50) * dof_class_cos[i];
      // }
      ...
      for (int i = 0; i < 3; i++)
        tmp_cur_vel_obs[i] = proj_grav[i];
      for (int i = 0; i < 12; i++)
        tmp_cur_vel_obs[3 + i] = state.motorState[sequence[i]].q;
      for (int i = 0; i < 12; i++)
        tmp_cur_vel_obs[15 + i] = state.motorState[sequence[i]].dq;
      for (int i = 0; i < 12; i++)
        tmp_cur_vel_obs[27 + i] = last_action[i];
      ...
      for (i = 0; i < 3; i++) {
        ...
        // tmp_obs.index_put_({start_index},
        // vel_output.index({i}).item().toFloat() * 2);
        start_index += 1;
      }
      for (int i = 0; i < 39; i++)
        tmp_last_vel_obs.index_put_({i}, tmp_cur_vel_obs.index({i}));
      ...
      for (int i = 0; i < 12; i++) {
        tmp_obs.index_put_({start_index},
                           state.motorState[sequence[i]].dq * 0.05);
        start_index += 1;
      }
    ...
    // for (i = 0; i < 12; i++ ){
    //     if (motiontime >= 400){
    //         action[i] = jointLinearInterpolation(last_action[i], action[i],
    //         0.2);
    //     }
    // }
    // for (i = 0; i < 12; i ++){
