...
extern const int LOW_CMD_LENGTH;    // shorter than sizeof(LowCmd),   bytes
                                    // compressed LowCmd length
extern const int LOW_STATE_LENGTH;  // shorter than sizeof(LowState), bytes
                                    // compressed LowState length
...
typedef struct {
  ...
  std::array<float, 3>
      gyroscope; // angular velocity （unit: rad/s)    (raw data)
  std::array<float, 3>
      accelerometer;        // m/(s2)                             (raw data)
...
typedef struct {
  ...
  uint8_t mode;
  float progress;
  uint8_t gaitType; // 0.idle  1.trot  2.trot running  3.climb stair  4.trot
                    // obstacle
  float footRaiseHeight; // (unit: m, default: 0.08m), foot up height while
                         // walking
  std::array<float, 3>
...
typedef struct {
  ...
  uint8_t
      mode; // 0. idle, default stand
            ...
            // 8. recovery stand
            // 9. backflip, reserve
            // 10. jumpYaw, only left direction. Note, to use this mode, you
            // need to set mode = 1 first.
            // 11. straightHand. Note, to use this mode, you need to set mode =
            // 1 first.
  ...
  float footRaiseHeight; // (unit: m, range: -0.06~0.03m, default: 0.09m), foot
                         // up height while walking, delta value
  float
      bodyHeight; // (unit: m, range: -0.13~0.03m, default: 0.31m), delta value
  std::array<float, 2>
      position; // (unit: m), desired position in inertial frame, reserve
