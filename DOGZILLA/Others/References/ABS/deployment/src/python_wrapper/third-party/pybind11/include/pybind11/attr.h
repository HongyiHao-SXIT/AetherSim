...
template <typename T> struct base {
  ...
  base() {} // NOLINT(modernize-use-equals-default): breaks MSVC 2015 when
            // adding an attribute
};
...
struct metaclass {
  ...
  metaclass() {} // NOLINT(modernize-use-equals-default): breaks MSVC 2015 when
                 // adding an attribute
...
struct function_record {
  ...
  /// True once a 'py::kw_only' is encountered (any following args are
  /// keyword-only)
...
inline void process_kw_only_arg(const arg &a, function_record *r) {
  if (!a.name || strlen(a.name) == 0)
    pybind11_fail("arg(): cannot specify an unnamed argument after an "
                  "kw_only() annotation");
  ++r->nargs_kw_only;
}
