...
public:
  ...
  ~loader_life_support() {
    ...
    // calls)
    if (stack.capacity() > 16 && !stack.empty() &&
...
struct is_copy_constructible<
    ...
    enable_if_t<all_of<std::is_copy_constructible<Container>,
                       ...
                       negation<std::is_same<
                           Container, typename Container::value_type>>>::value>>
...
type_caster<T, SFINAE> &load_type(type_caster<T, SFINAE> &conv,
  ...
  if (!conv.load(handle, true)) {
    ...
    throw cast_error("Unable to cast Python instance of type " +
                     (std::string)str(type::handle_of(handle)) +
                     " to C++ type '" + type_id<T>() + "'");
#endif
  }
  return conv;
...
detail::enable_if_t<!detail::move_never<T>::value, T> move(object &&obj) {
  if (obj.ref_count() > 1)
    ...
    throw cast_error("Unable to move from Python " +
                     (std::string)str(type::handle_of(obj)) +
...
/// the equivalent of an unnamed '*' argument (in Python 3)
struct kw_only {};
