...
public:
  ...
  `handle` or `object` subclass causes a call to ``__setitem__``.
  \endrst */
  ...
  // TODO PYBIND11_DEPRECATED("Call py::type::handle_of(h) or py::type::of(h)
...
public:
  ...
  /// If it is impossible to raise the currently-held error, such as in
  /// destructor, we can write it out using Python's unraisable hook
  /// (sys.unraisablehook). The error context should be some object whose repr()
  /// helps identify the location of the error. Python already knows the type
  /// and value of the error, so there is no need to repeat that. For example,
  /// __func__ could be helpful. After this call, the current object no longer
  /// stores the error variables, and neither does Python.
...
public:
  ...
  template <typename T> bool contains(T &&key) const {
    return PyDict_Contains(
               m_ptr, detail::object_or_cast(std::forward<T>(key)).ptr()) == 1;
  }
...
public:
  ...
  template <typename T> bool contains(T &&val) const {
    return PySet_Contains(
               m_ptr, detail::object_or_cast(std::forward<T>(val)).ptr()) == 1;
  }
};
...
public:
  ...
  static memoryview from_buffer(T *ptr, detail::any_container<ssize_t> shape,
    ...
    return memoryview::from_buffer(reinterpret_cast<void *>(ptr), sizeof(T),
                                   format_descriptor<T>::value, shape, strides,
                                   readonly);
  }
