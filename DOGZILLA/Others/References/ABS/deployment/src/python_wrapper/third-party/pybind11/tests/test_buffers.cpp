...
TEST_SUBMODULE(buffers, m) {
  ...
  py::class_<Matrix>(m, "Matrix", py::buffer_protocol())
      ...
      .def_buffer([](Matrix &m) -> py::buffer_info {
        return py::buffer_info(
            ...
            {sizeof(float) *
                 size_t(m.cols()), /* Strides (in bytes) for each index */
             sizeof(float)});
      });
  ...
  py::class_<BufferReadOnlySelect>(m, "BufferReadOnlySelect",
                                   py::buffer_protocol())
      .def(py::init<>())
  ...
  py::class_<py::buffer_info>(m, "buffer_info")
      ...
      .def("__repr__", [](py::handle self) {
        return py::str(
                   "itemsize={0.itemsize!r}, size={0.size!r}, "
                   "format={0.format!r}, ndim={0.ndim!r}, shape={0.shape!r}, "
                   "strides={0.strides!r}, readonly={0.readonly!r}")
            .format(self);
      });

  m.def("get_buffer_info", [](py::buffer buffer) { return buffer.request(); });
}
