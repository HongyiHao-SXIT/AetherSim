...
#if defined(_MSC_VER)
#pragma warning(disable : 4324) // warning C4324: structure was padded due to
                                // alignment specifier
#endif
...
TEST_SUBMODULE(class_, m) {
  ...
  m.def("get_type_of", [](py::object ob) { return py::type::of(ob); });

  m.def("get_type_classic", [](py::handle h) { return h.get_type(); });

  m.def("as_type", [](py::object ob) { return py::type(ob); });
  ...
  m.def("mismatched_holder_1", []() {
    ...
    py::class_<MismatchBase1, std::shared_ptr<MismatchBase1>>(mod,
                                                              "MismatchBase1");
  ...
  m.def("implicitly_convert_variable", [](py::object o) {
    ...
    // implicit conversion. This is valid when called inside a bound function
    // because the temp object is attached to the same life support system as
    // the arguments.
  ...
  // but we need a definition in a different compilation unit within the same
  // module:
  ...
  // We *don't* want to construct using braces when the given constructor
  // argument maps to a constructor, because brace initialization could go to
  // the wrong place (in particular when there is also an
  // `initializer_list<T>`-accept constructor):
  ...
  py::class_<Nested>(base, "Nested")
      ...
      .def(
          "fa", [](Nested &, int, NestBase &, Nested &) {}, "a"_a, "b"_a,
          "c"_a);
  ...
  py::class_<SamePointer, std::unique_ptr<SamePointer, py::nodelete>>(
      m, "SamePointer")
  ...
  py::class_<DerivedWithNested, BaseWithNested> derivedWithNested_class(
      m, "DerivedWithNested");
...
using DoesntBreak1 =
    py::class_<BreaksBase<1>, std::unique_ptr<BreaksBase<1>>, BreaksTramp<1>>;
using DoesntBreak2 =
    py::class_<BreaksBase<2>, BreaksTramp<2>, std::unique_ptr<BreaksBase<2>>>;
...
using DoesntBreak6 =
    py::class_<BreaksBase<6>, std::shared_ptr<BreaksBase<6>>, BreaksTramp<6>>;
using DoesntBreak7 =
    py::class_<BreaksBase<7>, BreaksTramp<7>, std::shared_ptr<BreaksBase<7>>>;
...
// leave them here, though, so that they can be manually tested by uncommenting
// them (and seeing that compilation failures occurs).
...
// BreaksTramp<-3>, BreaksTramp<-3>> Breaks3; CHECK_BROKEN(3);
//// Alias + 2 holders
// typedef py::class_<BreaksBase<-4>, std::unique_ptr<BreaksBase<-4>>,
// BreaksTramp<-4>, std::shared_ptr<BreaksBase<-4>>> Breaks4; CHECK_BROKEN(4);
