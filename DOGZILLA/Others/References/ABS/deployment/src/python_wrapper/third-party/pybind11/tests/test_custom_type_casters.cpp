...
TEST_SUBMODULE(custom_type_casters, m) {
  ...
  // no-conversion pass fail so that our call always ends up happening via the
  // second dispatch (the one that allows some conversion).
  class ArgInspector {
  public:
  ...
  m.def(
      "floats_only", [](double f) { return 0.5 * f; },
      py::arg("f").noconvert());
  ...
  m.def(
      ...
      py::return_value_policy::take_ownership); // Takes ownership: destroy when
                                                // finished
  m.def(
