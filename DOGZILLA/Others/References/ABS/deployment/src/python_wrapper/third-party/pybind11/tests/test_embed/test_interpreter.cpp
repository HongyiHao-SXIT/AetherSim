...
#ifdef _MSC_VER
// Silence MSVC C++17 deprecation warning from Catch regarding
// std::uncaught_exceptions (up to catch 2.0.1; this should be fixed in the next
// catch release after 2.0.1).
...
TEST_CASE("Restart the interpreter") {
  ...
  REQUIRE(py::module_::import("widget_module").attr("add")(1, 2).cast<int>() ==
          3);
  REQUIRE(has_pybind11_internals_builtin());
  REQUIRE(has_pybind11_internals_static());
  REQUIRE(py::module_::import("external_module")
              .attr("A")(123)
              .attr("value")
              .cast<int>() == 123);
  ...
  // finalize still gets the internals destroyed
  py::finalize_interpreter();
  py::initialize_interpreter();
  bool ran = false;
...
TEST_CASE("Subinterpreter") {
  ...
  // still works by returning from the static variable, i.e. all interpreters
  // share a single global pybind11::internals;
  REQUIRE_FALSE(has_pybind11_internals_builtin());
  REQUIRE(has_pybind11_internals_static());
  ...
  REQUIRE(py::hasattr(py::module_::import("widget_module"),
                      "extension_module_tag"));
}

TEST_CASE("Execution frame") {
  // When the interpreter is embedded, there is no execution frame, but
  // `py::exec` should still function by using reasonable globals:
  // `__main__.__dict__`.
...
TEST_CASE("Reload module from file") {
  ...
  // versions of the .py files generated below
