...
public:
  ...
  // TestFactory6 only when an alias is needed:
...
#define MAKE_TAG_TYPE(Name)                                                    \
  ...
  py::class_<TestFactory1>(m, "TestFactory1")
      ...
      .def(
          py::init([](pointer_tag) { return TestFactoryHelper::construct1(); }))
  ...
  py::class_<TestFactory2>(m, "TestFactory2")
      .def(py::init(
          [](pointer_tag, int v) { return TestFactoryHelper::construct2(v); }))
  ...
  py::class_<TestFactory3, std::shared_ptr<TestFactory3>>(m, "TestFactory3")
      .def(py::init(
          [](pointer_tag, int v) { return TestFactoryHelper::construct3(v); }))
      .def(py::init(
      ...
      .def(py::init(
          [](null_unique_ptr_tag) { return std::unique_ptr<TestFactory3>(); }))
      .def(py::init(
          [](null_shared_ptr_tag) { return std::shared_ptr<TestFactory3>(); }))
  ...
  py::class_<TestFactory7, PyTF7, std::shared_ptr<TestFactory7>>(m,
                                                                 "TestFactory7")
  ...
  py::class_<NoisyAlloc>(m, "NoisyAlloc")
      // Since these overloads have the same number of arguments, the dispatcher
      // will try each of them until the arguments convert.  Thus we can get a
      // pre-allocation here when passing a single non-integer:
      .def("__init__",
           [](NoisyAlloc *a, int i) {
             new (a) NoisyAlloc(i);
