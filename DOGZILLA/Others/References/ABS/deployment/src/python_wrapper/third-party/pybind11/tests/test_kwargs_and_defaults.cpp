...
TEST_SUBMODULE(kwargs_and_defaults, m) {
  ...
  m.def("args_function",
        [](py::args args) -> py::tuple { return std::move(args); });
...
// test_args_refcount
// PyPy needs a garbage collection to get the reference count values to match
// CPython's behaviour
#ifdef PYPY_VERSION
...
#define GC_IF_NEEDED
  ...
  m.def("args_refcount", [](py::args a) {
    ...
    for (size_t i = 0; i < a.size(); i++)
      // Use raw Python API here to avoid an extra, intermediate incref on the
      // tuple item:
  ...
  m.def("mixed_args_refcount", [](py::object o, py::args a) {
    ...
    for (size_t i = 0; i < a.size(); i++)
      ...
      // tuple item:
      t[i + 1] =
          (int)Py_REFCNT(PyTuple_GET_ITEM(a.ptr(), static_cast<ssize_t>(i)));
    return t;
  });

  // pybind11 won't allow these to be bound: args and kwargs, if present, must
  // be at the end. Uncomment these to test that the static_assert is indeed
  // working:
  ...
  m.def(
      "kw_only_all", [](int i, int j) { return py::make_tuple(i, j); },
      py::kw_only(), py::arg("i"), py::arg("j"));
  m.def(
      "kw_only_some",
      [](int i, int j, int k) { return py::make_tuple(i, j, k); }, py::arg(),
  ...
  m.def(
      "kw_only_mixed", [](int i, int j) { return py::make_tuple(i, j); }, "i"_a,
      py::kw_only(), "j"_a);
  m.def(
      "kw_only_plus_more",
      [](int i, int j, int k, py::kwargs kwargs) {
  ...
  m.def("register_invalid_kw_only", [](py::module_ m) {
    m.def(
        "bad_kw_only", [](int i, int j) { return py::make_tuple(i, j); },
        py::kw_only(), py::arg() /* invalid unnamed argument */, "j"_a);
  });

  // test_positional_only_args
  m.def(
      "pos_only_all", [](int i, int j) { return py::make_tuple(i, j); },
      py::arg("i"), py::arg("j"), py::pos_only());
  m.def(
      "pos_only_mix", [](int i, int j) { return py::make_tuple(i, j); },
      py::arg("i"), py::pos_only(), py::arg("j"));
  m.def(
      "pos_kw_only_mix",
      [](int i, int j, int k) { return py::make_tuple(i, j, k); }, py::arg("i"),
      py::pos_only(), py::arg("j"), py::kw_only(), py::arg("k"));
  m.def(
      "pos_only_def_mix",
      [](int i, int j, int k) { return py::make_tuple(i, j, k); }, py::arg("i"),
      py::arg("j") = 2, py::pos_only(), py::arg("k") = 3);
  ...
  m.def(
      "class_default_argument", [](py::object a) { return py::repr(a); },
