...
TEST_SUBMODULE(methods_and_attributes, m) {
  ...
  emna.def(py::init<>())
...
#else
      // Use both the traditional static_cast method and the C++11 compatible
      // overload_cast_
      .def("overloaded", overload_cast_<>()(&ExampleMandA::overloaded))
      .def("overloaded", overload_cast_<int>()(&ExampleMandA::overloaded))
      .def("overloaded",
           overload_cast_<int, float>()(&ExampleMandA::overloaded))
      ...
      .def("overloaded", static_cast<py::str (ExampleMandA::*)(float, float)>(
                             &ExampleMandA::overloaded))
      .def("overloaded_float",
           overload_cast_<float, float>()(&ExampleMandA::overloaded))
      .def("overloaded_const",
           overload_cast_<int>()(&ExampleMandA::overloaded, py::const_))
      .def("overloaded_const",
           overload_cast_<int, float>()(&ExampleMandA::overloaded, py::const_))
      .def("overloaded_const",
          ...
          []() {
            ...
            emna.def("overload_mixed1",
                     static_cast<py::str (ExampleMandA::*)(int, int)>(
                         &ExampleMandA::overloaded))
                .def_static("overload_mixed1", static_cast<py::str (*)(float)>(
                                                   &ExampleMandA::overloaded));
          })
      .def_static(
          ...
          []() {
            auto emna = py::reinterpret_borrow<py::class_<ExampleMandA>>(
                py::module_::import("pybind11_tests.methods_and_attributes")
                    .attr("ExampleMandA"));
            emna.def_static("overload_mixed2", static_cast<py::str (*)(float)>(
                                                   &ExampleMandA::overloaded))
                .def("overload_mixed2",
                     static_cast<py::str (ExampleMandA::*)(int, int)>(
                         &ExampleMandA::overloaded));
  ...
  py::class_<TestPropRVP>(m, "TestPropRVP")
      ...
      .def_property_static("static_rw_func",
                           py::cpp_function(static_get2, rvp_copy), static_set2)
  ...
  //
  // Issues #854/910: incompatible function args when member function/pointer is
  // in unregistered base class The methods and member pointers below actually
  // resolve to members/pointers in UnregisteredBase; before this test/fix they
  // would be registered via lambda with a first argument of an unregistered
  // type, and thus uncallable.
  py::class_<RegisteredDerived>(m, "RegisteredDerived")
      ...
      //static_assert if uncommented .def_readonly("fails", &UserType::value) //
  ...
  using Adapted = decltype(py::method_adaptor<RegisteredDerived>(
