...
// requiring extra space for holder constructed flags) works.
...
#define PYBIND11_BASEN(N)                                                      \
  ...
  // specified via template parameters because pybind has to know the types
  // involved; see discussion in #742 for details).
  ...
  // Test the case where not all base classes are specified, and where pybind11
  // requires the py::multiple_inheritance flag to perform proper casting
  // between types.
  struct Base1a {
    Base1a(int i) : i(i) {}
    int foo() { return i; }
  ...
  // type as being a known instance even when the pointer value is unequal (i.e.
  // due to a non-first multiple-inheritance base class):
  ...
  // Second, when returned a base class pointer to a derived instance, we cannot
  // assume that the pointer is `reinterpret_cast`able to the derived pointer
  // because, like above, the base class pointer could be offset.
  ...
  // derived type isn't pybind-registered (and uses multiple-inheritance to
  // offset the pybind base)
