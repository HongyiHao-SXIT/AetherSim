...
template <typename... Ix> ssize_t index_at(const arr &a, Ix... idx) {
  return a.index_at(idx...);
}
template <typename... Ix> ssize_t index_at_t(const arr_t &a, Ix... idx) {
  return a.index_at(idx...);
}
template <typename... Ix> ssize_t offset_at(const arr &a, Ix... idx) {
  return a.offset_at(idx...);
}
template <typename... Ix> ssize_t offset_at_t(const arr_t &a, Ix... idx) {
  return a.offset_at(idx...);
}
template <typename... Ix> ssize_t at_t(const arr_t &a, Ix... idx) {
  return a.at(idx...);
}
...
TEST_SUBMODULE(numpy_array, sm) {
  ...
  py::class_<DtypeCheck>(sm, "DtypeCheck")
      ...
      .def("__repr__", [](const DtypeCheck &self) {
        return py::str("<DtypeCheck numpy={} pybind11={}>")
            .format(self.numpy, self.pybind11);
      });
  ...
  py::class_<DtypeSizeCheck>(sm, "DtypeSizeCheck")
      ...
      .def("__repr__", [](const DtypeSizeCheck &self) {
        return py::
            str("<DtypeSizeCheck name='{}' size_cpp={} size_numpy={} dtype={}>")
  ...
  sm.def("make_empty_shaped_array",
         [] { return py::array(py::dtype("f"), {}, {}); });
  ...
  sm.def(
      "overloaded3", [](py::array_t<int>) { return "int"; },
      py::arg().noconvert());
  sm.def(
      "overloaded3", [](py::array_t<double>) { return "double"; },
      py::arg().noconvert());

  // Make sure we don't do unsafe coercion (e.g. float to int) when not using
  // forcecast, but rather that float gets converted via the safe (conversion to
  // double) overload:
  ...
  // overload matches without conversion)
  sm.def("overloaded5",
  ...
  // Issue (unnumbered; reported in #788): regression: initializer lists can be
  // ambiguous
  ...
  sm.def("index_using_ellipsis",
         [](py::array a) { return a[py::make_tuple(0, py::ellipsis(), 0)]; });

  // test_argument_conversions
  sm.def("accept_double", [](py::array_t<double, 0>) {}, py::arg("a"));
  sm.def(
      "accept_double_forcecast",
      [](py::array_t<double, py::array::forcecast>) {}, py::arg("a"));
  sm.def(
      "accept_double_c_style", [](py::array_t<double, py::array::c_style>) {},
      py::arg("a"));
  sm.def(
      ...
      py::arg("a"));
  sm.def(
      "accept_double_f_style", [](py::array_t<double, py::array::f_style>) {},
      py::arg("a"));
  sm.def(
      ...
      py::arg("a"));
  sm.def(
      "accept_double_noconvert", [](py::array_t<double, 0>) {},
      py::arg("a").noconvert());
  sm.def(
  ...
  sm.def(
      ...
      [](py::array_t<double, py::array::c_style>) {}, py::arg("a").noconvert());
  sm.def(
  ...
  sm.def(
      ...
      [](py::array_t<double, py::array::f_style>) {}, py::arg("a").noconvert());
  sm.def(
