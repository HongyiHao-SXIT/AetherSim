...
TEST_SUBMODULE(numpy_dtypes, m) {
  ...
  // typeinfo may be registered before the dtype descriptor for scalar casts to
  // work...
  py::class_<SimpleStruct>(m, "SimpleStruct")
      ...
      .def("astuple",
           [](const SimpleStruct &self) {
             return py::make_tuple(self.bool_, self.uint_, self.float_,
                                   self.ldbl_);
           })
      .def_static("fromtuple", [](const py::tuple tup) {
        ...
        return SimpleStruct{tup[0].cast<bool>(), tup[1].cast<uint32_t>(),
  ...
  // user that the struct is not a POD type
  ...
  m.def("compare_buffer_info", []() {
    ...
    list.append(py::bool_(
        py::detail::compare_buffer_info<CompareStruct>::compare(py::buffer_info(
            nullptr, sizeof(CompareStruct), "T{?:x:3xI:y:f:z:}", 1))));
    return list;
  });
  ...
  // test_str_leak
  m.def("dtype_wrapper",
        [](py::object d) { return py::dtype::from_args(std::move(d)); });
}
