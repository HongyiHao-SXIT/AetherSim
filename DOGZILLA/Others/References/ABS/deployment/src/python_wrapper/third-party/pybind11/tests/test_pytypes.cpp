...
TEST_SUBMODULE(pytypes, m) {
  ...
  m.def("print_none",
        [](py::none none) { py::print("none: {}"_s.format(none)); });
  ...
  m.def("set_contains",
        [](py::set set, py::object key) { return set.contains(key); });
  m.def("set_contains",
        [](py::set set, const char *key) { return set.contains(key); });
  ...
  m.def("dict_contains",
        [](py::dict dict, py::object val) { return dict.contains(val); });
  m.def("dict_contains",
        [](py::dict dict, const char *val) { return dict.contains(val); });
  ...
  m.def("return_capsule_with_name_and_destructor", []() {
    auto capsule = py::capsule(
        (void *)12345, "pointer type description", [](PyObject *ptr) {
          if (ptr) {
    ...
    py::print("created capsule ({}, '{}')"_s.format(result1 & result2 & result3,
                                                    capsule.name()));
    return capsule;
  });
  ...
  m.def("default_constructors", []() {
    return py::dict(
        "bytes"_a = py::bytes(), "str"_a = py::str(), "bool"_a = py::bool_(),
        "int"_a = py::int_(), "float"_a = py::float_(), "tuple"_a = py::tuple(),
        "list"_a = py::list(), "dict"_a = py::dict(), "set"_a = py::set());
  });

  m.def("converting_constructors", [](py::dict d) {
    return py::dict(
        "bytes"_a = py::bytes(d["bytes"]), "str"_a = py::str(d["str"]),
  ...
  m.def("cast_functions", [](py::dict d) {
    // When converting between Python types, obj.cast<T>() should be the same as
    // T(obj)
  ...
  m.def("nonconverting_constructor",
        [](std::string type, py::object value) -> py::object {
          if (type == "bytes") {
  ...
  m.def("print_function", []() {
    ...
    py::print("{a} + {b} = {c}"_s.format("a"_a = "py::print",
                                         "b"_a = "str.format", "c"_a = "this"));
  });
  ...
  m.def("test_memoryview_object",
        [](py::buffer b) { return py::memoryview(b); });

  m.def("test_memoryview_buffer_info",
        [](py::buffer b) { return py::memoryview(b.request()); });
  ...
  m.def("test_memoryview_from_buffer_nativeformat", []() {
    ...
    return py::memoryview::from_buffer(arr, sizeof(int32_t), format, {3},
                                       {sizeof(int32_t)});
  });
  ...
  m.def("test_memoryview_from_memory", []() {
    ...
    return py::memoryview::from_memory(buf, static_cast<ssize_t>(strlen(buf)));
  });
#endif
}
