...
TEST_SUBMODULE(stl, m) {
  ...
  m.def("load_vector",
        [](const std::vector<int> &v) { return v.at(0) == 1 && v.at(1) == 2; });
  // `std::vector<bool>` is special because it returns proxy objects instead of
  // references
  ...
  m.def("load_deque",
        [](const std::deque<int> &v) { return v.at(0) == 1 && v.at(1) == 2; });
  ...
  // NB: map and set keys are `const`, so while we technically do move them (as
  // `const Type &&`), casters don't typically do anything with that, which
  // means they fall to the `const Type &` caster.
  m.def("cast_rv_map", []() {
    return std::unordered_map<std::string, RValueCaster>{{"a", RValueCaster{}}};
  });
  m.def("cast_rv_nested", []() {
    std::vector<
        std::array<std::list<std::unordered_map<std::string, RValueCaster>>, 2>>
        v;
  ...
  static std::unordered_map<std::string,
                            std::vector<std::list<std::array<RValueCaster, 2>>>>
      lvn;
  ...
  using opt_exp_holder =
      OptionalHolder<std::experimental::optional, MoveOutDetector>;
  py::class_<opt_exp_holder>(m, "OptionalExpHolder",
  ...
  m.def(
      "stl_pass_by_pointer", [](std::vector<int> *v) { return *v; },
      "v"_a = nullptr);
