...
public:
  ...
  // strings, etc.) is a bit trickier, because the actual int& or std::string&
  // or whatever only exists temporarily, so we have to handle it specially in
  // the trampoline class (see below).
...
public:
  ...
  // We can return reference types for compatibility with C++ virtual interfaces
  // that do so, but note they have some significant limitations (see the
  // documentation).
...
// it should get moved when it is not referenced elsewhere, but copied if it is
// still referenced.
class Movable {
public:
...
// virtual approaches are rather long).
...
TEST_SUBMODULE(virtual_functions, m) {
  ...
  struct PyA : A {
    ...
    void f() override {
      py::print("PyA.f()");
      // This convolution just gives a `void`, but tests that PYBIND11_TYPE()
      // works to protect a type containing a ,
      PYBIND11_OVERRIDE(
          PYBIND11_TYPE(typename std::enable_if<true, void>::type), A, f);
    }
  };
  ...
  public:
    ...
    // Not allowed (uncommenting should hit a static_assert failure): we can't
    // get a reference to a python numeric value, since we only copy values in
    // the numeric type caster:
    //      std::string &str_ref() override { PYBIND11_OVERRIDE(std::string &,
...
// version and the templated trampoline versions mentioned in docs/advanced.rst.
//
...
//   or 4 classes and 11 methods (repeat).
...
//   there was no pure method (or no pure method override), the number would
//   drop down to the same 11 as the repeat approach).
...
public:
  using PyA_Tpl<Base>::PyA_Tpl; // Inherit constructors (via PyA_Tpl's inherited
                                // constructors)
  int unlucky_number() override {
...
// actually need these (we can use PyB_Tpl<C_Tpl> and PyB_Tpl<D_Tpl> for the
// trampoline classes instead):
/*
...
void initialize_inherited_virtuals(py::module_ &m) {
  ...
  // Fix issue #1454 (crash when acquiring/releasing GIL on another thread in
  // Python 2.7)
